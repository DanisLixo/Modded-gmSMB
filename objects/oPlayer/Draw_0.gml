...
{
	if global.paletteindex = 2 {palindex = 1;} else {palindex = 2;}
}
else {palindex = global.paletteindex;}
...
if firetimer > 0 
&& (sprite_exists(asset_get_index(spritestring)) || ds_map_find_value(global.moddedSprites, spritestring) != undefined) 
&& (char != "Sonic" || char = "Sonic" and state != ps.jump)
{
	{
		firedraw = true;
		
		var fs = ms("sMario_b_fire");
...
{firedraw = false; draw_sprite_ext(spr,ind,x,y+yoff,round(image_xscale)*scale,image_yscale*scale,image_angle,image_blend,image_alpha);}
shader_reset()
...
if instance_exists(oIsArena)
{
	...
	if state != ps.die {
		...
		if instance_place(x,y,oSuperstardropped)
		{global.stars ++; instance_destroy(instance_place(x,y,oSuperstardropped))}
	}
}