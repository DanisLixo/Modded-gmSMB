...
function player_collision()
{
	...
	if (spintimer > 0)
	{obj = collision_rectangle(bbox_left - 12, bbox_top, bbox_right + 12, bbox_bottom, oParenemy, false, true);}
	if (starman > 0)
	{obj = collision_rectangle(bbox_left - 6, bbox_top, bbox_right + 6, bbox_bottom, oParenemy, false, true);}
	
	if obj 
	&& (starman > 0 || spin = true || shoulderbash > 0 || spintimer > 0 || pepdancin > 0) 
	&& obj.state != es.die && obj.state != es.stomp
	{
		if (spintimer > 0)
		{
			obj = collision_rectangle(bbox_left - 12, bbox_top, bbox_right + 12, bbox_bottom, oParenemy, false, true)
			...
			with(obj) 
			{
				...
				if stomptype <= 0 || stomptype = 3 {
					state = es.die;
				
					if (object_index == oGoomba) points(100,true);
					else if (object_index == oHammerbro || object_index == oBlooper) points(1000,true);
			...
			show_debug_message("Enemy found in Mario's scope, " + string(obj) + "'s being hitted")
	...
	if obj {
		if obj.state = es.patrol || obj.state = es.patrolwinged {
			...
			|| (char == "Sonic" && state == ps.jump)
			{
				if obj.stomptype = -1 || (global.environment = e.underwater && global.nouwstomp)
				{gethit = 1;}
					...
					if obj.object_index = oHammerbro 
					{points(5 + combo, true);}
					else if obj.object_index = oLakitu 
					{points(4 + combo, true);}
					else if obj.cheeptype != 2 {
						if obj.state != es.shell 
						{points(combo, true);}
					}
