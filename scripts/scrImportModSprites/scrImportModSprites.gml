function ImportModSounds(_directory) {
	show_debug_message("Finding sounds or sum")
	var directoryArray = []
	...
	for (var i = 0; i < array_length(directoryArray); i++) {
		...
		while mySoundCurrent != "" {
			...
			if ds_map_find_value(global.moddedSounds, soundName) != undefined 
				show_debug_message("Added sound: " + audio_get_name(soundAdded) + " to ds map global.moddedSounds under " + soundName)
			mySoundCurrent = file_find_next()
		}
	}
}

function ImportDemos() {
	show_debug_message("Finding demos ig")
	var directoryArray = []
	var myDirArray = file_find_first("demos\\demo*", fa_directory)
	while myDirArray != "" {
		array_push(directoryArray, myDirArray)
		myDirArray = file_find_next()
	}
	for (var i = 0; i < array_length(directoryArray); i++) {
		global.demoCount++;
		show_debug_message("ADDED DEMO: "+ myDirArray + " " + string(global.demoCount))
	}
}
	...
	for (var i = 0; i < array_length(directoryArray); i++) {
		...
		for (var j = 0; j < array_length(sprArray); j++) {
			...
			if ds_map_find_value(global.moddedSprites, mySpriteNoPng) != undefined 
				show_debug_message("Found super cool mod sprite! moer: "+mySpriteNoPng)
			else
			show_debug_message("Want moer!!! (Find was done)")
		}
		ini_close()	
	}
}
...
function ImportModGSpr(_directory){
	...
	for (var i = 0; i < array_length(directoryArray); i++) 
	{
		...
		for (var j = 0; j < array_length(sprArray); j++)
		{
			...
			ds_map_add(global.moddedSprites, mySpriteNoPng, mySpr)
			if ds_map_find_value(global.moddedSprites, mySpriteNoPng) != undefined 
				show_debug_message("Found gun sprite! moer: "+mySpriteNoPng)
			else
			show_debug_message("want moer!!! (Find was done)")
		}
		ini_close()	
