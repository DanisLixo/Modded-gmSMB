...
function ps_transform()
{
	if spr != ms("sMario_s_grow")
	{
		ind = 0;
		
		if powerup == "c" {sfx(sndFeather,1);}
		else {sfx(sndPowerup,1);}
	}
	...
	if kjp || kj and jumpbuffer >= 1 {jumpbuffer++}
...
function ps_shrink()
{
	...
	{
		if global.pupleave {
			if powerup == "f" && !(instance_exists(oHat) || instance_exists(oHatThrow))
			{instance_create_depth(x,y,depth,oPudestroy).sprite_index = sFireflower_wooble;}
			if powerup == "b" 
			{
				instance_create_depth(x,y,depth,oPudestroy).sprite_index = sMushbottom;
				instance_create_depth(x,y,depth,oPudestroy).sprite_index = sMushhead;
			}
			if powerup == "c" 
